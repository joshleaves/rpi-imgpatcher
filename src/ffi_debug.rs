...
#[unsafe(no_mangle)]
pub extern "C" fn rpi_imgpatcher_last_error_free(error: *const c_char) {
  // Manual changes done by the user: empty implementation
  // since it's now returning a pointer to a thread-local.
  let _ = error;
}
